...
public class TeamRegistrationController {
    ...
    public ResponseEntity<ResponseStatus<TeamRegistrationResponseDto>> registerTeam(
            @PathVariable Integer hackathonId,
            @PathVariable Integer teamId,
            @RequestParam Integer userId) {
        TeamRegistrationResponseDto response = teamRegistrationService.registerTeam(hackathonId, teamId, userId);
    ...
    public ResponseEntity<ResponseStatus<String>> withdrawTeam(
            @PathVariable Integer hackathonId,
            @PathVariable Integer teamId,
            @RequestParam Integer userId) {
        teamRegistrationService.withdrawTeam(hackathonId, teamId,userId);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(ResponseStatus.success("Team registration withdrawn successfully"));
    }
    ...
    public ResponseEntity<ResponseStatus<TeamRegistrationResponseDto>> updateRegistrationStatus(
        ...
        TeamRegistrationResponseDto response = teamRegistrationService
                .updateRegistrationStatus(hackathonId, teamId, userId, status);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(ResponseStatus.success(response));
    }
    ...
    getUserParticipatedHackathons(@PathVariable Integer userId) {
        List<TeamRegistrationResponseDto> response = teamRegistrationService
                .getUserParticipatedHackathons(userId);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(ResponseStatus.success(response));
    }
